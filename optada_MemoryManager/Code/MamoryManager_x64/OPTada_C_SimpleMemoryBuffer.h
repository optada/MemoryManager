...
protected:
	...
	OPTadaS_MemoryCellElement * FirstCell_Buffer = NULL;   // Указатель на первую ячейку
	OPTadaS_MemoryCellElement * FreeCells_Buffer = NULL;   // Буфеер свободных ячеек
	OPTadaS_MemoryCellElement * LockedCells_Buffer = NULL; // Буффер заблокированных ячеек
	OPTadaC_MemoryCells_StaticCyclicBuffer * Elem_Buffer = NULL; // Буффер с ячейками - элементами (для выдачи и получения)
